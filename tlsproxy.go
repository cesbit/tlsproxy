...
func startSocket(clientPort int, targetPort int, config *tls.Config) {
...
func main() {
	...
	cert, err := tls.LoadX509KeyPair(certFileEnv, keyFileEnv)
	if err != nil {
	...
	for _, m := range match {
		if m[3] == "" {
			...
			go startSocket(port, port, config)
		} else {
			clientPort, _ := strconv.Atoi(m[1])
			targetPort, _ := strconv.Atoi(m[3])
			go startSocket(clientPort, targetPort, config)
		}
	}
